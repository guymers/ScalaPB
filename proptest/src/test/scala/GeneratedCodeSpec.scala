...
class GeneratedCodeSpec extends AnyPropSpec with ScalaCheckDrivenPropertyChecks with Matchers {
  val printer = GTextFormat.printer().escapingNonAscii(false)
  ...
  property("Java and Scala protos are equivalent", Slow) {
    forAll(SchemaGenerators.genCompiledSchema, workers(1), minSuccessful(20)) {
      (schema: CompiledSchema) =>
        forAll(GenData.genMessageValueInstance(schema.rootNode)) { case (message, messageValue) =>
          ...
          try {
            ...
            companion.fromAscii(printer.printToString(javaProto)) should be(scalaProto)
